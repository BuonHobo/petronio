...
impl Transition {
    ...
    fn is_active(&self, vect: &[i32]) -> bool {
        if !self.processing { //La transazione deve essere libera, non occupata
            self.input.iter().all(|(i, u)| {
                if u.is_negative() {  //u negativo vuol dire che è inibitore
                    !(vect[*i] >= -*u as i32)
                } else {
                    vect[*i] >= *u as i32
                }
    ...
    fn enable(&self, vect: &mut [i32]) -> () {
        self.input.iter()
            .for_each(|(i, u)| if !u.is_negative() { vect[*i] -= *u as i32 });
        self.output.iter()
            .for_each(|(i, u)| if !u.is_negative() { vect[*i] += *u as i32 });
    }
}
...
fn main() {
    //Dati della rete
    let places: Vec<i32> = vec![1, 1, 0, 1, 0, 0, 0, 0, 0]; //marcatura iniziale
    let trans: Vec<Transition> = vec![ //transizioni
                                       //(ID, lista con (posto entrante, peso), lista con (posto uscente, peso), durata).
                                       Transition::new(1, &[(0, 1)], &[(2, 1)], 1),
                                       Transition::new(2, &[(2, 1), (3, 1)], &[(5, 1)], 0),
                                       Transition::new(3, &[(5, 1)], &[(7, 1), (3, 1)], 2),
                                       Transition::new(4, &[(7, 1)], &[(0, 1)], 3),
                                       Transition::new(5, &[(1, 1)], &[(4, 1)], 4),
                                       Transition::new(6, &[(4, 1), (3, 1)], &[(6, 1)], 0),
                                       Transition::new(7, &[(6, 1)], &[(8, 1), (3, 1)], 5),
                                       Transition::new(8, &[(8, 1)], &[(1, 1)], 6),
    ];
    let n=20;

    //Preparo i dati al multithreading
    let shared_trans: Vec<Arc<Mutex<Transition>>> = trans.iter()
    ...
    while i <= n {
        ...
        let active_trans: Vec<Arc<Mutex<Transition>>> = shared_trans.iter()
            .filter(|t| lockunwrap(t).is_active(lockunwrap(&shared_places).as_slice()))
            .map(|t| t.clone())
            .collect();

        let attive = active_trans.iter().map(|t| lockunwrap(t).id).collect::<Vec<u8>>();
